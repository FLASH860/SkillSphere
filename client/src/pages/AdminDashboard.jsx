  ...
  return (
    <DashboardShell title="Admin Dashboard">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        {statCards.map((c) => (
          <div key={c.label} className="bg-white/5 border border-white/10 rounded-lg p-5">
            <p className="text-white/40 text-xs font-mono uppercase">{c.label}</p>
            <p className={`text-3xl font-serif mt-2 ${c.amber ? 'text-amber' : ''}`}>{c.value}</p>
          </div>
        ))}
      </div>

      <div className="flex items-center justify-between bg-white/5 border border-white/10 rounded-lg px-5 py-4 mb-8">
        <div>
          <p className="text-white/90 text-sm font-medium">Manual Gig Approval</p>
        ...
        <button
          ...
          className={`relative w-12 h-6 rounded-full transition ${
            settings?.manualGigApprovalEnabled ? 'bg-sage' : 'bg-white/20'
          }`}
        >
          <span
      ...
      <div className="flex gap-4 border-b border-white/10 mb-6">
        <button
          ...
          className={`pb-2 text-sm font-mono ${tab === 'users' ? 'text-amber border-b-2 border-amber' : 'text-white/40'}`}
        >
          Users
        </button>
        <button
          onClick={() => setTab('gigs')}
          className={`pb-2 text-sm font-mono ${tab === 'gigs' ? 'text-amber border-b-2 border-amber' : 'text-white/40'}`}
        >
          Gigs
        </button>
        <button
          onClick={() => setTab('approvals')}
          className={`pb-2 text-sm font-mono flex items-center gap-2 ${tab === 'approvals' ? 'text-amber border-b-2 border-amber' : 'text-white/40'}`}
        >
          Approvals
        ...
        <button
          ...
          className={`pb-2 text-sm font-mono ${tab === 'disputes' ? 'text-amber border-b-2 border-amber' : 'text-white/40'}`}
        >
          Disputes
        </button>
      </div>
      ...
      {tab === 'users' && (
        <div className="border border-white/10 rounded-lg overflow-hidden">
            ...
            <tbody>
              {users?.map((u) => (
                <tr key={u._id} className="border-t border-white/5">
                  ...
                  <td className="px-4 py-3 capitalize text-white/70">{u.role}</td>
          ...
          {!users?.length && (
            <p className="text-white/40 text-sm p-6 text-center">No users found.</p>
          )}
        </div>
      )}
      ...
      {tab === 'gigs' && (
        <div className="border border-white/10 rounded-lg overflow-hidden">
            ...
            <tbody>
              {gigs?.map((g) => (
                <tr key={g._id} className="border-t border-white/5">
                  <td className="px-4 py-3 text-white/90">{g.title}</td>
                  <td className="px-4 py-3 text-white/50">{g.client?.name || '—'}</td>
          ...
          {!gigs?.length && (
            <p className="text-white/40 text-sm p-6 text-center">No gigs found.</p>
          )}
        </div>
      )}
      ...
      {tab === 'approvals' && (
        <div className="border border-white/10 rounded-lg overflow-hidden">
          ...
          {!pendingGigs?.length && (
            <p className="text-white/40 text-sm p-6 text-center">No gigs awaiting approval.</p>
          )}
        </div>
      )}
      ...
      {tab === 'disputes' && (
        <div className="border border-white/10 rounded-lg overflow-hidden">
            ...
            <tbody>
              {disputes?.map((d) => (
                <tr key={d._id} className="border-t border-white/5 align-top">
                  <td className="px-4 py-3 text-white/90">{d.gig?.title || '—'}</td>
                  <td className="px-4 py-3 text-white/50 text-xs font-mono">{d.raisedBy?.name || '—'}</td>
                  <td className="px-4 py-3">
                    <button
          ...
          {!disputes?.length && (
            <p className="text-white/40 text-sm p-6 text-center">No disputes filed.</p>
          )}
        </div>
      )}
      ...
      {viewingReason && (
        ...
        >
          <div
            className="bg-ink border border-white/10 rounded-lg p-6 max-w-lg w-full max-h-[70vh] overflow-y-auto"
          ...
          >
            ...
            <p className="text-white/80 text-sm whitespace-pre-wrap mb-4">{viewingReason.reason}</p>
            {viewingReason.evidenceUrls?.length > 0 && (
              <div>
                <p className="text-white/40 text-xs font-mono uppercase mb-2">Evidence</p>
                <div className="flex flex-wrap gap-2">
                  {viewingReason.evidenceUrls.map((url, idx) => (
                    <a key={idx} href={url} target="_blank" rel="noreferrer">
                      <img src={url} alt="evidence" className="w-20 h-20 object-cover rounded border border-white/20 hover:border-amber transition" />
                    </a>
                  ))}
                </div>
              </div>
      ...
      {confirmingToggle && (
        ...
        >
          <div
            className="bg-ink border border-white/10 rounded-lg p-6 max-w-sm w-full"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="flex gap-2 justify-end">
              <button
                onClick={() => setConfirmingToggle(false)}
                className="text-xs font-mono border border-white/20 px-4 py-2 rounded hover:border-white/40"
              >
                Cancel
              </button>
              <button
