    ...
    <DashboardShell title="Client Dashboard">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-white/5 border border-white/10 rounded-lg p-5">
          <p className="text-white/40 text-xs font-mono uppercase">Active Gigs</p>
          <p className="text-3xl font-serif mt-2">{activeCount}</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-lg p-5">
          <p className="text-white/40 text-xs font-mono uppercase">Total Gigs Posted</p>
          <p className="text-3xl font-serif mt-2 text-amber">{gigs?.length || 0}</p>
        </div>
        <div className="bg-white/5 border border-white/10 rounded-lg p-5">
          <p className="text-white/40 text-xs font-mono uppercase">Total Spent</p>
          <p className="text-3xl font-serif mt-2">$0</p>
        </div>
      </div>
      ...
      {stats && (stats.statusBreakdown?.length > 0 || stats.gigsPerMonth?.length > 0) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {stats.statusBreakdown?.length > 0 && (
            <div className="bg-white/5 border border-white/10 rounded-lg p-5">
              <p className="text-white/50 text-xs font-mono uppercase mb-3">Gig Status Breakdown</p>
          ...
          {stats.gigsPerMonth?.length > 0 && (
            <div className="bg-white/5 border border-white/10 rounded-lg p-5">
              <p className="text-white/50 text-xs font-mono uppercase mb-3">Gigs Posted Per Month</p>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={stats.gigsPerMonth}>
                  ...
                  <Tooltip contentStyle={{ background: '#14192B', border: '1px solid #ffffff20' }} />
                  <Bar dataKey="gigs" fill="#E8A33D" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          )}
        ...
        {gigs?.map((gig) => (
          ...
          >
            ...
            <div className="flex gap-2">
              <Link
                to={`/gigs/${gig._id}`}
                className="text-sm border border-white/20 px-3 py-1.5 rounded hover:border-amber hover:text-amber transition"
              >
                View Gig
              </Link>
              {gig.status === 'open' && (
